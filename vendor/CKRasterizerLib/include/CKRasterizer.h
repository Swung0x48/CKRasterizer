...
/*	rasterizers as a common interface to access a rendering device.		 */
/*																		 */
/*	+ A Rasterizer DLL must provide the implementation for the 3 base 	 */
/*	classes by overloading them:										 */
/*		- CKRasterizer: Top level class (One instance per rasterizer)    */
/*		which upon creation should check for available drivers.		     */
/*																		 */
/*		- CKRasterizerDriver: One instance should be created for each	 */
/*		available driver (ie graphic card,software,hardware,etc.). It is */
/*		used to store capabilities and create rendering contexts		 */
/*																		 */
...
public:
    ...
    XSArray<CKBYTE> m_ObjectsIndex; // A List of BYTE mask to keep track of the existing object
...
public:
    ...
    virtual CKBOOL
    Clear(CKDWORD Flags = CKRST_CTXCLEAR_ALL, CKDWORD Ccol = 0, float Z = 1.0f, CKDWORD Stencil = 0, int RectCount = 0,
    ...
    virtual CKBOOL
    DrawPrimitive(VXPRIMITIVETYPE pType, CKWORD *indices, int indexcount, VxDrawPrimitiveData *data) { return FALSE; }
    ...
    virtual CKBOOL
    DrawPrimitiveVBIB(VXPRIMITIVETYPE pType, CKDWORD VB, CKDWORD IB, CKDWORD MinVIndex, CKDWORD VertexCount,
    ...
    virtual CKBOOL
    SetTargetTexture(CKDWORD TextureObject, int Width = 0, int Height = 0, CKRST_CUBEFACE Face = CKRST_CUBEFACE_XPOS,
...
public:
    ...
    //---- For DX8 rasterizer this ensure that vertex shader are supported :
