...
{
    HRESULT hr;
    // currently only supporting CBV/SRV/UAV // not any more
    //assert(type == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    m_HeapDesc.NumDescriptors = size;
    m_HeapDesc.Type = type;
    /*m_HeapDesc.Flags = (type == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) ? 
...
HRESULT CKDX12DescriptorRing::CreateSampler(const D3D12_SAMPLER_DESC &desc, CD3DX12_GPU_DESCRIPTOR_HANDLE &gpuHandle)
{
    assert(m_HeapDesc.Type == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
    HRESULT hr = S_OK;
    auto offset = CKDX12RingBufferBase::Allocate(1);
    if (offset == InvalidOffset)
        return E_OUTOFMEMORY;
