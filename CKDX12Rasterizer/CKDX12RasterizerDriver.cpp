...
CKDX12RasterizerDriver::CKDX12RasterizerDriver(CKDX12Rasterizer *rst) {
    m_Owner = rst;
}

CKDX12RasterizerDriver::~CKDX12RasterizerDriver() {
...
CKRasterizerContext *CKDX12RasterizerDriver::CreateContext() {
    if (!m_Contexts.IsEmpty())
        return m_Contexts.Front();
    auto* ctx = new CKDX12RasterizerContext();
    ctx->m_Driver = this;
    ctx->m_Owner = static_cast<CKDX12Rasterizer *>(m_Owner);
    m_Contexts.PushBack(ctx);
    return ctx;
}

CKBOOL CKDX12RasterizerDriver::DestroyContext(CKRasterizerContext *Context) {
    static_cast<CKDX12RasterizerContext *>(Context)->m_InCreateDestroy = TRUE;
...
CKBOOL CKDX12RasterizerDriver::InitializeCaps(Microsoft::WRL::ComPtr<IDXGIAdapter1> Adapter,
                                              ...
                                              bool HighPerformance)
{
    ...
    m_3DCaps.MaxNumberTextureStage = MAX_TEX_STAGES; // D3D12_COMMONSHADER_SAMPLER_SLOT_COUNT; //We do 2 for now.
    ...
    if (m_Output)
    {
        ...
        int len = WideCharToMultiByte(CP_ACP, 0, m_OutputDesc.DeviceName, wcslen(m_OutputDesc.DeviceName), nullptr, 0,
                                      nullptr, nullptr);
        WideCharToMultiByte(CP_ACP, 0, m_OutputDesc.DeviceName, wcslen(m_OutputDesc.DeviceName), buf, len, nullptr,
                            nullptr);
    } else
    {
        strcpy(buf, "(Headless)");
